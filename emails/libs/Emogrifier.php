...
class Emogrifier {
	 ...
	 *
	 * @param string $html
	 *        	the HTML to emogrify, must be UTF-8-encoded
	 * @param string $css
	 *        	the CSS to merge, must be UTF-8-encoded
	 */
	 ...
	 *
	 * @param int $key
	 *        	the cache key, must be CACHE_KEY_CSS, CACHE_KEY_SELECTOR, CACHE_KEY_XPATH
	...
	private function removeInvisibleNodes(\DOMXPath $xPath) {
		...
		foreach ( $nodesWithStyleDisplayNone as $node ) {
			if ($node->parentNode && is_callable ( array( 
					$node->parentNode,
					'removeChild' 
			) )) {
	...
	private function translateCssToXpath($cssSelector) {
		...
		if (! isset ( $this->caches [self::CACHE_KEY_XPATH] [$xPathKey] )) {
			...
			$xPathWithIdAttributeMatchers = preg_replace_callback ( self::ID_ATTRIBUTE_MATCHER, array( 
					$this,
					'matchIdAttributes' 
			), $roughXpath );
			$xPathWithIdAttributeAndClassMatchers = preg_replace_callback ( self::CLASS_ATTRIBUTE_MATCHER, array(
					$this,
					'matchClassAttributes' 
			), $xPathWithIdAttributeMatchers );
			// Advanced selectors are going to require a bit more advanced emogrification.
			// When we required PHP 5.3, we could do this with closures.
			$xPathWithIdAttributeAndClassMatchers = preg_replace_callback ( '/([^\\/]+):nth-child\\(\\s*(odd|even|[+\\-]?\\d|[+\\-]?\\d?n(\\s*[+\\-]\\s*\\d)?)\\s*\\)/i', array( 
					$this,
					'translateNthChild' 
			), $xPathWithIdAttributeAndClassMatchers );
			$finalXpath = preg_replace_callback ( '/([^\\/]+):nth-of-type\\(\s*(odd|even|[+\\-]?\\d|[+\\-]?\\d?n(\\s*[+\\-]\\s*\\d)?)\\s*\\)/i', array( 
					$this,
					'translateNthOfType' 
			), $xPathWithIdAttributeAndClassMatchers );
	...
	private function parseNth(array $match) {
		if (in_array ( strtolower ( $match [2] ), array( 
				'even',
				'odd' 
		), true )) {
			// we have "even" or "odd"
	 ...
	 * "font-weight" => "bold"
	 *
	 * @param string $cssDeclarationsBlock
